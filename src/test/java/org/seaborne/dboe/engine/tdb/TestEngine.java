...
public class TestEngine extends Assert {
    ...
    private static Dataset create(String datafile) {
        ...
        try {
            ...
            // @before/@after.
            if ( datafile != null )
                Txn.executeWrite(ds,  ()->RDFDataMgr.read(ds, DIR+"/"+datafile));
    ...
    @Test public void exec11() {
        testOp("(sequence (bgp (?s :p ?v) (?s :p 123)) (bgp (?s :p ?v))  )", "data1.ttl");
    }

    @Test public void exec12() {
        testOp("(filter (= ?v 123) (bgp (?s :p ?o) (?s :p ?v)) )", "data1.ttl");
    }

    @Test public void exec13() {
        testOp("(filter (= ?v 123) (bgp (?s :p ?v) (?s :p ?o)) )", "data1.ttl");
    }
    ...
    private void testOp(String opString, String datafile) {
        ...
        try {
            Txn.executeRead(dsg, ()-> testOp(op, dsg, dsgMem));
        }
